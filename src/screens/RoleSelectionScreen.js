...
export default function RoleSelectionScreen() {
    ...
    const fetchProfiles = async (retryCount = 0) => {
        try {
            ...
            if (!familyId) {
                if (retryCount < 3) {
                    console.log(`Buscando família... Tentativa ${retryCount + 1}`);
            ...
            if ((!profilesData || profilesData.length === 0) && retryCount < 3) {
                console.log(`Buscando perfis... Tentativa ${retryCount + 1}`);
                setTimeout(() => fetchProfiles(retryCount + 1), 1000);
                return;
            }
    ...
    const renderProfileCard = ({ item }) => {
        ...
        const bgColor = isCaptain ? '#FFFBEB' : '#F0FDF4';
        const borderColor = isCaptain ? '#FCD34D' : '#6EE7B7';
        const iconName = isCaptain ? "crown" : "star-face";

        // Puxando o título dinamicamente do banco de dados (item.title_archetype)
        const labelText = isCaptain
            ? (item.title_archetype ? item.title_archetype.toUpperCase() : 'CAPITÃO')
            : 'AVENTUREIRO';
        ...
        <View style={styles.container}>
            ...
            <View style={styles.listContainer}>
                <FlatList
                    ...
                    refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => { setRefreshing(true); fetchProfiles(0); }} tintColor="#10B981"/>}
...
const styles = StyleSheet.create({
    ...
    pinInput: {
        ...
        textAlign: 'center', fontSize: 34, fontFamily: FONTS.bold, color: '#059669', letterSpacing: 10
    },
