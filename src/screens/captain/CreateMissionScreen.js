...
export default function CreateMissionScreen() {
    ...
    useEffect(() => {
        if (currentFamilyId) fetchProfiles(currentFamilyId);
    }, [currentFamilyId]);
                    ...
                    <View style={styles.formContainer}>
                        ...
                        <View style={styles.inputGroup}>
                            ...
                            <View style={styles.tabSwitchContainer}>
                                <TouchableOpacity style={[styles.tabSwitchBtn, !isRecurring && styles.tabSwitchActive]} onPress={()=>setIsRecurring(false)}>
                                    <MaterialCommunityIcons name="calendar" size={16} color={!isRecurring ? '#fff' : '#10B981'} />
                                ...
                                <TouchableOpacity style={[styles.tabSwitchBtn, isRecurring && styles.tabSwitchActive]} onPress={()=>setIsRecurring(true)}>
                                    <MaterialCommunityIcons name="calendar-sync" size={16} color={isRecurring ? '#fff' : '#10B981'} />
                            ...
                            {!isRecurring ? (
                                ...
                                <View style={{marginTop: 10}}>
                                    ...
                                    <View style={styles.weekRow}>
                                        {WEEKDAYS.map(d => (
                                            <TouchableOpacity key={d.id} style={[styles.dayCircle, selectedDays.includes(d.id) && {backgroundColor: '#10B981', borderColor: '#10B981'}]} onPress={()=>toggleDay(d.id)}>
                            ...
                            <View style={{flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: useCritical ? 10 : 0}}>
                                <View style={{flexDirection: 'row', alignItems: 'center'}}>
                                    <View style={[styles.luckyIcon, useCritical && {backgroundColor: '#FFF'}]}>
                                        <MaterialCommunityIcons name="treasure-chest" size={24} color={useCritical ? '#F59E0B' : '#CBD5E1'} />
                                    </View>
                                    <View style={{marginLeft: 12}}>
                                        <Text style={[styles.luckyTitle, useCritical && {color: '#B45309'}]}>Tesouro Adicional</Text>
                                ...
                                <TouchableOpacity onPress={() => setUseCritical(!useCritical)}>
                                    <MaterialCommunityIcons name={useCritical ? "toggle-switch" : "toggle-switch-off-outline"} size={40} color={useCritical ? '#F59E0B' : '#CBD5E1'} />
                                </TouchableOpacity>
                            </View>
                        ...
                        {!isEditingTemplate && (
                            <TouchableOpacity style={[styles.templateBox, saveAsTemplate && styles.templateBoxActive]} activeOpacity={0.9} onPress={() => setSaveAsTemplate(!saveAsTemplate)}>
                                <View style={[styles.checkbox, saveAsTemplate && {backgroundColor: '#10B981', borderColor: '#10B981'}]}>
                                ...
                                <MaterialCommunityIcons name="flash" size={20} color={saveAsTemplate ? '#10B981' : "#94A3B8"} />
                            </TouchableOpacity>
                        )}
                    ...
                    <View style={styles.modalContent}>
                        ...
                        {profiles.map(p=>(
                            <TouchableOpacity key={p.id} style={styles.modalOption} onPress={()=>{setAssignee(p.id); setAssigneeName(p.name); setShowAssigneeModal(false)}}>
                                ...
                                {assignee === p.id && <MaterialCommunityIcons name="check" size={20} color="#10B981"/>}
                            </TouchableOpacity>
                        ))}
                        <TouchableOpacity style={styles.modalOption} onPress={()=>{setAssignee(null); setAssigneeName('TODOS'); setShowAssigneeModal(false)}}>
                            ...
                            {assignee === null && <MaterialCommunityIcons name="check" size={20} color="#10B981"/>}
                        </TouchableOpacity>
...
const styles = StyleSheet.create({
    ...
    toggleBtn: { flex: 1, paddingVertical: 12, alignItems: 'center', borderWidth: 1, borderColor: '#10B981', borderRadius: 24, backgroundColor: '#F8FAFC' },
    ...
    luckyBoxActive: { borderColor: '#F59E0B', backgroundColor: '#FFFBEB' },
    ...
    miniBtnActive: { backgroundColor: '#10B981' },
    ...
    probBtnActive: { backgroundColor: '#F59E0B', borderColor: '#F59E0B' },
    ...
    dayCircle: { width: 36, height: 36, borderRadius: 18, borderWidth: 1, borderColor: '#10B981', justifyContent: 'center', alignItems: 'center', backgroundColor: '#FFF' },
    ...
    templateBox: { flexDirection: 'row', backgroundColor: '#F0F9FF', padding: 15, borderRadius: 20, borderWidth: 1, borderColor: '#10B981', alignItems: 'center', marginBottom: 20 },
    templateBoxActive: { backgroundColor: '#F0FDF4', borderColor: '#10B981' },
    ...
    createBtnShadow: { position: 'absolute', top: 4, left: 0, width: '100%', height: '100%', backgroundColor: '#059669', borderRadius: 16 },
    createBtnFront: { width: '100%', height: '100%', backgroundColor: '#10B981', borderRadius: 16, borderWidth: 1, borderColor: 'rgba(255,255,255,0.2)', flexDirection: 'row', alignItems: 'center', justifyContent: 'center', marginTop: -2 },
    btnText: { fontFamily: FONTS.bold, color: '#FFF', fontSize: 14, letterSpacing: 1 },
    modalOverlay: { flex: 1, backgroundColor: 'rgba(0,0,0,0.5)', justifyContent: 'center', alignItems: 'center', padding: 20 },
    modalContent: { width: '100%', backgroundColor: '#FFF', borderRadius: 24, padding: 20, borderWidth: 1, borderColor: '#10B981' },
